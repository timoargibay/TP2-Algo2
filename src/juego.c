...
bool elegir_pokemons(const lista_t *pokemons, lista_t *pokemons_elegidos, unsigned int *semilla)
{
        if(pokemons == NULL || pokemons_elegidos == NULL)
                return false;
         
        size_t cantidad = lista_cantidad(pokemons);
        size_t cantidad_a_elegir = ((CANT_FILAS * CANT_COLUMNAS) %2 == 0) ? CANT_FILAS * CANT_COLUMNAS : CANT_FILAS*CANT_COLUMNAS -1;
        ...
        size_t posiciones_elegidas[cantidad];
        for(i = 0; i < cantidad; i++)
                posiciones_elegidas[i] = i;

        mezclar_fisher_yates(posiciones_elegidas, cantidad);
...
void imprimir_carta_revelada(const char *texto, char salida[CARTA_ALTO][CARTA_ANCHO + 1]) {
    snprintf(salida[0], CARTA_ANCHO + 1, "+-------+");
...
void mostrar_tablero(lista_t *cartas, size_t filas, size_t columnas)
{
        ...
        size_t total = columnas * CARTA_ANCHO + (columnas-1) * ESPACIO;
        size_t padding = (ANCHO_IMAGINARIO > total) ? (ANCHO_IMAGINARIO - total) / 2 : 0;
        ...
        for(i = 0; i < filas; i++) {
                ...
                for(j = 0; j < CARTA_ALTO; j++) {
                        ...
                        for(k = 0; k < columnas; k++) {
                                carta_t *carta_actual = lista_buscar_elemento(cartas, i*columnas+k);
                                if(carta_actual == NULL)
                                        ...
                                        imprimir_carta_revelada(carta_actual->pokemon_carta->nombre, diseño);
                                else
                                        imprimir_carta_oculta(diseño);
...
carta_t *elegir_carta(lista_t *cartas, size_t filas, size_t columnas, carta_t *eleccion_previa)
{
        ...
        while(seleccion_valida == false) {
                free(input);
                printf("\nSeleccione una carta (ej: A1, B3, C2): ");
                input = leer_linea_archivo(stdin);
                printf(ANSI_CLEAR_LINE);

                if(input == NULL || leer_coordenada(input, &fila, &columna) == false) {
                ...
                carta_elegida = lista_buscar_elemento(cartas, fila * columnas + columna);
...
int juego(lista_t *pokemons_disponibles, unsigned int semilla)
{
        ...
        size_t cantidad_pokemons = lista_cantidad(pokemons_disponibles);
        lista_iterador_t *iterador = NULL;
        
        if(pokemons_elegidos == NULL || cartas == NULL || cartas_mezcladas == NULL) {
        ...
        if(elegir_pokemons(pokemons_disponibles, pokemons_elegidos, &semilla) == false) {
                lista_destruir(pokemons_elegidos);
                lista_destruir(cartas);
                lista_destruir(cartas_mezcladas);
                return 1;
        ...
        for(i = 0; i < cantidad_cartas; i++) {
                lista_agregar(cartas_mezcladas, lista_buscar_elemento(cartas, array_pos_cartas[i]));
        }
        ...
        while(cartas_eliminadas < lista_cantidad(cartas)) {
                ...
                carta1 = elegir_carta(cartas, CANT_FILAS, CANT_COLUMNAS, NULL);
                carta1->revelada = true;
                mostrar_tablero(cartas, CANT_FILAS, CANT_COLUMNAS);
                carta2 = elegir_carta(cartas, CANT_FILAS, CANT_COLUMNAS, carta1);
                carta2->revelada = true;
                mostrar_tablero(cartas, CANT_FILAS, CANT_COLUMNAS);
        
                if(carta1->pokemon_carta->id == carta2->pokemon_carta->id) {
                        printf("\n" ANSI_COLOR_GREEN "¡Correcto! :D\n" ANSI_COLOR_RESET);
        ...
        printf("Ganaste! :) *escena de evangelion de todos aplaudiendo*\n");
