...
void prueba_menu_null()
{
	...
	menu_destruir(prueba);
	pa2m_afirmar(prueba != NULL, "Al crear un menu sin titulo ni estilo, no hay problemas");
}

void prueba_agregar_opcion_null()
{
	pa2m_afirmar(menu_agregar_opcion(NULL, "Pruebaaa", 'K', ANSI_COLOR_BLUE) == false, "Agregar opcion a menu NULL da false");
}

void prueba_modificar_titulo()
{
	pa2m_afirmar(menu_modificar_titulo(NULL, "Hola", NULL) == false, "Modificar titulo de menu NULL da false");
}

void prueba_mostrar_menu_null()
{
	pa2m_afirmar(menu_mostrar(NULL, false) == 0, "Mostrar menu NULL devuelve 0");
}
